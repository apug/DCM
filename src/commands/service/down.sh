...
if [ -n "${args[services]}" ]; then
  ...
  compose_cmd="docker compose stop ${containers_to_stop[@]}"
  msg_info "Stopping services: ${service_names[@]}"
else
  # Stop all services
  compose_cmd="docker compose down"
  msg_info "Stopping all services..."
fi
...
if [ $exit_code -eq 0 ]; then
  ...
  msg_error "Failed to stop services (exit code: $exit_code)"
  exit $exit_code
fi
